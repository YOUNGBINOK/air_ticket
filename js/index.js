...
async function getData() {
  // 출발정보
  const departuresApiUrl = `http://apis.data.go.kr/B551177/StatusOfPassengerFlightsOdp/getPassengerDeparturesOdp?serviceKey=${API_KEY}&from_time=0000&to_time=2400&lang=K&type=json`;
  // 도착정보
...
//   }
// });
async function departureInfoData() {
  try {
    let departureData = await departureInfo.response.body.items;
    let ticket = document.getElementById("ticket");
    //for (let i = 0; i < departureData.length; i++) {
    for (let i = 0; i < 10; i++) {
      let ticketLeft = document.createElement("div");
      let ticketTop = document.createElement("div");
      let logo = document.createElement("h1");
      let ticketTitle = document.createElement("h2");
      let boardingPass = document.createElement("p");

      let content = document.createElement("div");
      let city = document.createElement("div");
      let startAirport = document.createElement("h3");
      let finalAirport = document.createElement("h3");
      let departureAirportCode = document.createElement("p");

      ticket.appendChild(ticketLeft);
      ticketLeft.appendChild(ticketTop);
      ticketTop.appendChild(logo);
      ticketTop.appendChild(ticketTitle);
      ticketTop.appendChild(boardingPass);

      ticket.appendChild(content);
      content.appendChild(city);
      city.appendChild(startAirport);
      city.appendChild(finalAirport);
      city.appendChild(departureAirportCode);

      ticketLeft.classList.add("ticket_left");
      ticketTop.classList.add("ticket_top");
      logo.classList.add("logo");
      ticketTitle.classList.add("ticket_title");
      boardingPass.classList.add("boarding_pass");
      content.classList.add("content");
      city.classList.add("city");
      startAirport.classList.add("start_airport");
      finalAirport.classList.add("final_airport");
      departureAirportCode.classList.add("departure_airport_code");

      logo.textContent = `로고`;
      ticketTitle.textContent = departureData[i].airline;
      boardingPass.textContent = `BOARDING PASS`;
